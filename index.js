...
<body>
	<div class="container">
		<h1>Cloud Drive</h1>
		<p class="upload-status" id="uploadStatus">Ready to upload your files!</p>
	...
	<script>
		function loadFileList() {
			...
			savedFiles.forEach((file, index) => {
				...
				li.innerHTML = \`
					...
					<span class="file-info">Size: \${(file.Size / 1024).toFixed(2)} KB | Uploaded: \${new Date(file.Date).toLocaleDateString()}</span>
		...
		async function handleFileUpload(file) {
			...
			try {
				...
				const savedFiles = JSON.parse(localStorage.getItem('uploadedFiles')) || [];
				savedFiles.push({ Name: file.name, Url: result.Hash, Size: file.size, Date: new Date().toISOString() });
				localStorage.setItem('uploadedFiles', JSON.stringify(savedFiles));
